...
public class ItemPool {
	...
	public void addChest(int numItems, int tier, Inventory chestInv, Chest chest) {
		chest.update(true);
		p.sendMessage("TIER IS " + tier);
		for (int i = 0; i < numItems; i++) {
			int index = rand.nextInt(27) + 1;
				if(chestInv.getItem(index) != null) {
					index = indexCheck(chestInv);
				}
			int chance = rand.nextInt(100) + 1;
			p.sendMessage("RAND VALUE " + chance);
			if (tier == 1) {
				if (chance <= 30) {
					chestInv.setItem(index, map.get("1wood"));
				} else if (chance <= 55) {
					chestInv.setItem(index, map.get("1stone"));
				} else if (chance <= 70) {
					chestInv.setItem(index, map.get("1coal"));
				} else if (chance <= 83) {
					chestInv.setItem(index, map.get("1gold"));
				} else if (chance <= 92) {
					chestInv.setItem(index, map.get("1diamond"));
				} else if (chance <= 100) {
					chestInv.setItem(index, map.get("1emerald"));
				}
			}

			if (tier == 2) {
				if (chance <= 30) {
					chestInv.setItem(index, map.get("2wood"));
				} else if (chance <= 55) {
					chestInv.setItem(index, map.get("2stone"));
				} else if (chance <= 70) {
					chestInv.setItem(index, map.get("2coal"));
				} else if (chance <= 83) {
					chestInv.setItem(index, map.get("2gold"));
				} else if (chance <= 92) {
					chestInv.setItem(index, map.get("2diamond"));
				} else if (chance <= 100) {
					chestInv.setItem(index, map.get("2emerald"));
				}
			}

			if (tier == 3) {
				if (chance <= 30) {
					chestInv.setItem(index, map.get("3wood"));
				} else if (chance <= 55) {
					chestInv.setItem(index, map.get("3stone"));
				} else if (chance <= 70) {
					chestInv.setItem(index, map.get("3coal"));
				} else if (chance <= 83) {
					chestInv.setItem(index, map.get("3gold"));
				} else if (chance <= 92) {
					chestInv.setItem(index, map.get("3diamond"));
				} else if (chance <= 100) {
					chestInv.setItem(index, map.get("3emerald"));
				}
			}
		}
	}
